...
const AvatarRoot = React.forwardRef<HTMLDivElement, AvatarRootProps>(
	({ asChild, children, size, color, className, placeholderType = 'user', ...rest }, forwardedRef) => {
		...
		if (!children) {
			return (
				<div
					...
					{...rest}
				>
					<AvatarImage asChild>{placeholderType === 'company' ? <IconEmptyCompany /> : <IconEmptyUser />}</AvatarImage>
				</div>
			);
		}
...
const AvatarBrandLogo = React.forwardRef<HTMLImageElement, AvatarBrandLogoProps>(
	({ asChild, className, ...rest }, forwardedRef) => {
		...
		return (
			<Component
				ref={forwardedRef}
				className={cnExt('box-content size-6 rounded-full border-2 border-bg-white-0', className)}
				{...rest}
			/>
		);
	}
...
function AvatarNotification({ className, ...rest }: React.HTMLAttributes<HTMLDivElement>) {
	return (
		<div
			className={cnExt('box-content size-3 rounded-full border-2 border-bg-white-0 bg-error-base', className)}
			{...rest}
		/>
	);
}
